...
t = 3 # Number of base field elements (state size)
c = 2  # Number of elements dedicated to the capacity (random elements)
...
Fp = GF(p, 'a')
L = []
L = [[0 for i in range(2)], [1 for i in range(2)]]
for j in range(1):
    L.append([Fp.random_element() for i in range(2)])

# L.append(Fp.random_element())
...
R = []
for l in L:
    R.append(griffin_sponge(parameters, l, 8))
# R.append(griffin_sponge(parameters, L, 8))
print(R)
'''
