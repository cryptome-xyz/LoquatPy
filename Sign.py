...
def loquat_sign_phase_1(sk, n, m, p, Fp, Fp2, listH, eta, kappa, ldt_lists, algebraic_hash, loquat_hash, tree_cap,
    ...
    for j in range(n):
        ...
        list_r.extend(rs)
        for r in rs:
            T.append(floor(1 / 2 * (1 - kronecker(r, p))))
            vec_c_j.append(Fp2(sk * r))
            vec_c_j.append(Fp2(r))
...
def loquat_sign_phase_5(Fp2, algebraic_hash, loquat_hash, loquat_expand, sigma_4, h_3, n, m, salt, sigma_2, vec_lambda,
    ...
    #print(h_3[0] * mu + value_S)
    numerator_poly = size_H * poly_f_prime - size_H * poly_Z_H * poly_h - (h_3[0] * mu + value_S)
...
def loquat_sign_phase_7(algebraic_hash, loquat_expand, h_i, kappa, Fp2, n, eval_c_prime, eval_s, eval_h, tree_c,
    ...
    query_leaf_c = [[eval_c_prime[j][item] for item in queries] for j in range(n)]
    ...
    for i in range(0, r + 1):
        ...
        for ind in range(len(queries)):
            if i == 0:
                temp.append(MT_open(queries[ind], tree_f[i]))
            else:
                index = floor(queries[ind] / 4 ** i)
                if index not in temp_ind:
                    temp.append(MT_open(index, tree_f[i]))
                    temp_ind.append(index)
                    temp2.update({index: vec_ldt[i - 1][index]})
        auth_r.append(temp)
        if len(temp2) != 0:
            query_r.append(temp2)
...
def loquat_sign(pp, sk, msg):
    ...
    eval_c_prime, tree_c, poly_c_prime, vec_r, sigma_1 = loquat_sign_phase_1(sk, n, m, p, Fp, Fp2, listH,
                                                                             eta,
    ...
    vec_f_0, h_4 = loquat_sign_phase_5(Fp2, algebraic_hash, loquat_hash, loquat_expand, sigma_4, h_3, n, m, salt,
                                       ...
                                       eval_c_prime, eta, eval_s, eval_h, poly_c_prime, poly_s, rho_star, listH)
